...
if /i "%userInput%"=="Y" (
	...
	echo Command Executed. Please restart to take effect.
	echo.
	goto restartpc
...
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    echo System detected : x64.
) else if "%PROCESSOR_ARCHITECTURE%"=="x86" (
    echo System detected : x86.
) else (
...
if /i "%userInput%"=="Y" (
	...
	echo.
	echo msiexec.exe /i "C:\GLPI-Agent-1.7.3-%global_installtype%.msi" /norestart SERVER="https://helpdeskict.moh.gov.my/" RUNNOW=1 ADDLOCAL=ALL ADD_FIREWALL_EXCEPTION=1 NO_SSL_CHECK=1 tag="%global_tagid%" > runspai.bat
	echo runspai.bat created in %CD% as backup.
	echo.
) else if /i "%userInput%"=="N" (
	goto menu
) else (
